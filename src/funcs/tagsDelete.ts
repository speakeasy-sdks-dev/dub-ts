...
export async function tagsDelete(
  ...
  >
> {
  const input: operations.DeleteTagRequest = {
    id: id,
  };

  const parsed = safeParse(
    input,
    (value) => operations.DeleteTagRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/tags/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.DeleteTagResponseBody$inboundSchema),
